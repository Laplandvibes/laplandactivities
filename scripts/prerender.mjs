// scripts/prerender.mjs  (laplandactivities)
//
// Non-invasive, react-snap–style post-build prerender for the LaplandActivities
...
//      visit every route × every locale (incl. dynamic destination/category
//      slugs discovered from src/data),
...
// ---- Discover dynamic slugs from source (so routes never drift) ------------
async function discoverSlugs(file) {
  try {
...
function distFileFor(prefix, route) {
  ...
  if (route) {
    for (const part of route.split('/').filter(Boolean)) segs.push(part)
  }
...
async function main() {
  ...
  const destSlugs = await discoverSlugs('destinations.ts')
  const catSlugs = await discoverSlugs('categories.ts')
  const ROUTES = [
    ...STATIC_ROUTES,
    ...destSlugs.map((s) => `/destinations/${s}`),
    ...catSlugs.map((s) => `/categories/${s}`),
  ]
  console.log(
    `[prerender] routes: ${STATIC_ROUTES.length} static + ${destSlugs.length} destinations + ` +
      `${catSlugs.length} categories = ${ROUTES.length} × ${LOCALE_PREFIXES.length} locales = ` +
